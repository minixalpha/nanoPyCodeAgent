...
为了探索 read 工具是否比 bash 更有效，进一步检查了五个项目中的测试、设计文档、系统提示、变更记录和源码注释，并专门搜索了 `eval`、`bench`、`benchmark` 目录以及同时出现 Read 与 `cat`/Bash 的测试。没有发现真正的端到端 A/B 评测：没有测试让同一模型完成同一任务，分别只提供专用 Read 或 Bash，再统计任务成功率、token、延迟或错误率。

现有证据能支持的命题要窄得多：

- 专用 Read 可以更稳定地提供有界分页、确定的续读位置、结构化权限、原生多模态内容和可恢复错误；
- 某些框架级优化只对专用 Read 生效，确实能减少重复上下文或 prompt cache 成本；
- 这些行为由大量单元测试和回归测试覆盖；
- 但它们不能直接推出“使用 Read 的 Agent 总体任务成功率高于使用 Bash 的 Agent”。

系统提示中“优先使用 Read”的文字只能证明产品设计意图，不能单独视为效果证据。

因此，以下我们对比一下 read 工具有哪些好处，哪些坏处。
