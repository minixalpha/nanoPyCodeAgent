...
所以下一步分成两件独立的工作：先把 `httpx` 补成直接依赖，保证容器安装不依赖传递关系；再设计内部标准事件、`stream-json` 和 trajectory writer，让 Harbor adapter 可以把 native trajectory 转成 ATIF，并可靠回填 token、cost 和步骤统计。这些属于对已跑通链路的可安装性与可观测性增强，不是本次「让最简单 benchmark 完整跑起来」的完成条件。
