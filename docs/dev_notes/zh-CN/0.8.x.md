...
「没做完也 exit 0」是最反直觉、也最容易做反的一条：Harbor 把 agent 命令包在 `set -o pipefail` 下执行，非零退出码直接抛 `NonZeroAgentExitCodeError`，整个 trial 判成 agent 失败，还可能触发重试白烧钱。任务做没做成该由 verifier 从 `/logs/verifier/` 的 reward 文件判，不该由 agent 的退出码判。
...
任务从管道进去、文件真的建出来、退出码是 0，这一步就算完成。此后 nanoPyCodeAgent 才第一次具备「被脚本调用」的能力，也才谈得上接 benchmark。
