...
6. **接通 CLI 与 Harbor。** 增加独立的 `--trajectory PATH`，不改变 stdout 的 `--output-format`，并拒绝 `-` 与 stdout 争用；Harbor adapter 只负责传入日志路径、声明并读取 ATIF，再回填 steps、tokens 和 cost，不再做 native trajectory 转换。
