...
- Versioned Native Events and a per-run internal Event Journal. Model, tool,
  ...
  ordering, UTC timestamps, durations, usage, provider response metadata, and
  explicit truncation metadata. Journals are stored as sensitive user-only
  files while the existing text stdout remains unchanged as an event
  projection.
